...
?>
	<table class="tbl" border="0" cellspacing="1" cellpadding="5" width="75%">
		...
		<tr>
			<td class="hr" align="left"><?php echo htmlspecialchars("Task Name")."&nbsp;" ?></td>
			<td class="dr" align="left">
            	<input type="text" name="task_name" class="uppercase" size="30" maxlength="30" value="<?php echo str_replace('"', '&quot;', trim($row["task_name"])) ?>">
                <?php echo htmlspecialchars("*")."&nbsp;" ?>
            </td>
		</tr>
		<tr>
			<td class="hr" align="left"><?php echo htmlspecialchars("Task Description")."&nbsp;" ?></td>
			<td class="dr" align="left">
            	<input type="text" name="task_description" class="uppercase" size="50" maxlength="50" value="<?php echo str_replace('"', '&quot;', trim($row["task_description"])) ?>">
...
?>
	<table class="tbl" border="0" cellspacing="1" cellpadding="5"width="50%">
		<tr>
			<td class="hr" align="left"><?php echo htmlspecialchars("Task Name")."&nbsp;" ?></td>
			<td class="dr" align="left"><?php echo htmlspecialchars(strtoupper($row["task_name"])) ?></td>
		</tr>
		<tr>
			<td class="hr" align="left"><?php echo htmlspecialchars("Task Description")."&nbsp;" ?></td>
			<td class="dr" align="left"><?php echo htmlspecialchars(strtoupper($row["task_description"])) ?></td>
		</tr>
...
?>
	<table class="bd" border="0" cellspacing="1" cellpadding="4">
		<tr>
			<td><input type="button" name="btnBack" value="Back" onClick="javascript:LoadDiv('<?php echo CENTER_DIV ?>','task/task.php?status=true')"></td>
			<?php 
			if ($recid > 0) 
			{ 
			?>
				<td><input type="button" name="btnPrior" value="Prior Record" onClick="javascript:LoadDiv('<?php echo CENTER_DIV ?>','task/task.php?status=true&a=<?php echo $a ?>&recid=<?php echo $recid - 1 ?>')"></td>
			<?php 
			} 
			if ($recid < $count - 1) { ?>
				<td><input type="button" name="btnNext" value="Next Record" onClick="javascript:LoadDiv('<?php echo CENTER_DIV ?>','task/task.php?status=true&a=<?php echo $a ?>&recid=<?php echo $recid + 1 ?>')"></td>
	  <?php } ?>
	   </tr>
	</table>
	<hr size="1" noshade>
...
?>
	...
	<hr size="1" noshade>
	<form name="category" enctype="multipart/form-data" action="task/processTask.php?status=true&action=add" method="post">
		...
		?>
		<p><input type="button" name="btnAdd" id="btnAdd" value="Save" onClick="if(task_validate()==true){javascript: formget(this.form, 'task/processTask.php');}"></p>
...
?>
	...
	<table class="bd" border="0" cellspacing="1" cellpadding="4">
		<tr>
			<td>
            <input type="button" name="btnEdit" value="Edit Record" onClick="javascript:LoadDiv('<?php echo CENTER_DIV ?>','task/task.php?status=true&a=edit&recid=<?php echo $recid ?>')">
            </td>
...
?>
	<br>
	<form name="category" enctype="multipart/form-data" action="task/processTask.php?status=true&action=update" method="post">
		<input type="hidden" name="sql" value="update">
        <input type="hidden" name="action" value="update">
		<input type="hidden" name="cat_id" value="<?php echo $row["task_code"] ?>">
		<?php showroweditor($row, true); ?>
		<p>
        <input type="button" name="btnedit" id="btnedit" value="Update" onClick="if(task_validate()==true){javascript: formget(this.form, 'task/processTask.php');}">
        </p>
        <div id="output" style="color: blue;"></div>
	</form>
<?php
...
?>
	<br>
	<form action="task/processTask.php?status=true&action=delete" method="post">
...
function sql_select()
{
	...
	if (isset($filterstr) && $filterstr!='' && isset($filterfield) && $filterfield!='') {
    	...
    	$sql .= " and (`task_name` like '" .$filterstr ."') or (`task_description` like '" .$filterstr ."') ";
  	}
...
function sql_getrecordcount()
{
	...
	global $cattypeId;
  	...
  	$filterstr = sqlstr($filter);
  	if ( $filterstr!='') $filterstr = "%" .$filterstr ."%";
  		$sql = "SELECT COUNT(*) FROM tbl_task";
