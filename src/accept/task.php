...
if(isset($_SESSION['User_Name']) and $_SESSION['User_Name']!=""){
	...
	if(isset($_GET["cmbtaskname"])) $taskvalue = $_GET["cmbtaskname"];
	...
	if (!isset($taskvalue) && isset($_SESSION["taskvalue"])) $taskvalue = $_SESSION["taskvalue"];
...
?>
	<form name="frmListProduct" action="" method="post">
		<table border="0" cellspacing="1" cellpadding="4" width="100%" bgcolor="#EAEAEA">
			<tr>
				...
				<input type="button" name="action" value="Apply" onClick="javascript:SearchData(this.form,'accept/task.php?status=true&a=filter')" >
				<input type="button" name="action" value="Reset" onClick="javascript:LoadDiv('<?php echo CENTER_DIV ?>','accept/task.php?status=true&a=reset')">
                </td>
	...
	<hr size="1" noshade>
    <form name="frmLead" id="frmLead" action="accept/task.php?status=true&a=del" method="post">
	<table class="bd" border="0" cellspacing="1" cellpadding="4" width="100%">
		<tr>
			
            <td align="right">Rows Per Page
    	     <select name="pageperrecord" onChange="javascript:LoadDiv('<?php echo CENTER_DIV ?>','accept/task.php?status=true&pageId='+this.value)">
	...
	<table class="tbl" border="0" cellspacing="1" cellpadding="5"width="100%">
    	<tr>
			...
			<td class="hr">
            	<a class="hr" href="javascript:LoadDiv('<?php echo CENTER_DIV ?>','accept/task.php?status=true&order=<?php echo "customer_name" ?>&type=<?php echo $ordtypestr ?>')">
					<?php echo htmlspecialchars("Company Name") ?>
                </a>
            </td>
            <td class="hr">
            	<a class="hr" href="javascript:LoadDiv('<?php echo CENTER_DIV ?>','accept/task.php?status=true&order=<?php echo "lead_date" ?>&type=<?php echo $ordtypestr ?>')">
					<?php echo htmlspecialchars("Lead Date") ?>
                </a>
            </td>
			
            <td class="hr">
            	<a class="hr" href="javascript:LoadDiv('<?php echo CENTER_DIV ?>','accept/task.php?status=true&order=<?php echo "proposal_quote_date" ?>&type=<?php echo $ordtypestr ?>')">
					<?php echo htmlspecialchars("Quote Date") ?>
                </a>
            </td>
            
            <td class="hr">
          	<a class="hr" href="javascript:LoadDiv('<?php echo CENTER_DIV ?>','accept/task.php?status=true&order=<?php echo "task_status" ?>&type=<?php echo $ordtypestr ?>')">
	...
	<tr class="<?php echo $style ?>" style="cursor:pointer">
	  ...
	  <td align="left" onClick="javascript:LoadDiv('<?php echo CENTER_DIV ?>','accept/task.php?status=true&a=view&recid=<?php echo $i ?>')">
	...
	<?php 
		...
		showpagenav($page, $pagecount,$pagerange,'accept/task.php'); 
} 
?></center>
<?php
...
?>
	...
	<table border="0" cellspacing="1" cellpadding="5" width="100%" class="hrhighlightblue">
		...
		<table  border="0" cellspacing="1" cellpadding="5" width="100%" class="hrhighlightblue">
        <?php 
			if(taskdataFound($leadid) && $a == "Edit"){	
		?>
        	<tr>
            	<td colspan="2">
         ...
         <?php 
		 	}elseif(taskdataFound($leadid) && ($recordstatus == "editrecord" || $recordstatus == "confirmrecord" || $recordstatus == "deleterecord")){
				$edit_qry = "select a.id,a.lead_id,a.quot_id,b.task_code,b.task_name,a.task_summary 
							 from tbl_lead_task a,tbl_task b where a.task_code=b.task_code and a.id='$recordid'";
				$edit_res = mysqli_query($conn,$edit_qry);
				if(mysqli_num_rows($edit_res)>0){
					$edit_row = mysqli_fetch_assoc($edit_res);
					$taskname = $edit_row["task_name"];
					$tasksummary = $edit_row["task_summary"];
					$taskid = $edit_row["task_code"];
					$tasksummary = $tasksummary." - ".date("d-m-Y",strtotime($today));
				}
			}
			$taskList = buildTaskOptions('$taskid');
		 ?>
            <tr>     
                <td align="left" width="15%">Task Name</td>
                <input type="hidden" name="recordstatus" value="<?php echo $recordstatus; ?>">
                <input type="hidden" name="recordid" value="<?php echo $recordid; ?>">
                <td align="left">
                <?php if ($taskvalue == "newTask"){ ?>
                	<input type="hidden" name="taskstatus" value="new">
                	<input type="text" name="cmbtaskname" size="30" maxlength="30">
                <?php }else{ ?> 
				    <input type="hidden" name="taskstatus" value="exists">
                	<select name="cmbtaskname">
                    	<option value="">--Choose One--</option>
                        <option value="newTask" onChange="javascript:LoadDiv('<?php echo CENTER_DIV ?>','accept/task.php?status=true&a=edit&cmbtaskname='+this.value)">--New Task--</option>
                    	<?php echo $taskList; ?>
                    </select>
                <?php 
					}
            ...
            <tr>     
                <td align="left" width="15%">Task Summary</td>
                <td align="left">
                	<?php if(isset($tasksummary) && (strlen($tasksummary)>1) && $tasksummary != ""){ ?>
                		<textarea name="txttasksummary" rows="4" cols="45"><?=$tasksummary?></textarea>
                    <?php }else{ ?>
                    	<textarea name="txttasksummary" rows="4" cols="45"></textarea>
                    <?php } ?>
                <?php echo htmlspecialchars("*")."&nbsp;" ?>
                </td>
          	</tr>
...
function dataEdit($rid,$totamt,$damt){
    ...
    <?php
		...
		while($ret_row= mysqli_fetch_assoc($ret_res)){
			...
			echo "<td>";
			?>
            <a href="javascript:LoadDiv('<?php echo CENTER_DIV ?>', 'accept/task.php?status=true&a=edit&recordstatus=updaterecord&recordid=<?php echo $recid; ?>&eqty='+document.frmLead.eprod_qty.value+'&ettax='+document.frmLead.eprod_tax.value+'&etdiscount='+document.frmLead.eprod_discount.value+'&etprice=<?php echo $price; ?>')">Update</a>
			<?php
			echo "</td><td>";
			?>
			<a href="javascript:LoadDiv('<?php echo CENTER_DIV ?>', 'accept/task.php?status=true&a=edit&recordstatus=cancelrecord&recordid=<?php echo $recid; ?>')">Cancel</a>
			<?php
			echo "</td>";
			echo "</tr>";
...
function dataRetrieve($lid){
	global $conn;
	global $recid;
	$ret_qry= "select b.id,b.task_code,b.task_summary,b.task_date,b.record_status,c.task_name
    ...
    <?php
		...
		while($ret_row= mysqli_fetch_assoc($ret_res)){
			...
			echo "<td align='right'>".date("d-m-Y",strtotime($ret_row["task_date"]))."</td>";
			if($ret_row["record_status"] != "confirmrecord"){
			?>
            <td align="center">
            <a href="javascript:LoadDiv('<?php echo CENTER_DIV ?>','accept/task.php?status=true&a=edit&recordstatus=editrecord&recordid=<?php echo $rec_id; ?>&recid=<?php echo $recid; ?>')">Edit</a>
            </td>
            <td align="center">
            <a href="javascript:LoadDiv('<?php echo CENTER_DIV ?>','accept/task.php?status=true&a=edit&recordstatus=deleterecord&recordid=<?php echo $rec_id; ?>&recid=<?php echo $recid; ?>')">Delete</a>
			
			</td>
            <td align="center">
            <a href="javascript:LoadDiv('<?php echo CENTER_DIV ?>','accept/task.php?status=true&a=edit&recordstatus=confirmrecord&recordid=<?php echo $rec_id; ?>&recid=<?php echo $recid; ?>')">Confirm</a>
            </td>
            </tr>
        <?php
			}else{
...
/*function dataRetrieve($lid,$qid){
	global $conn;
	$ret_qry= "select b.id,c.task_name,b.task_summary
				from tbl_lead_master a,tbl_lead_task b,tbl_task c
				where a.lead_id=b.lead_id and a.quot_id=b.quot_id and b.task_code=c.task_code and b.lead_id='$lid' and b.quot_id='$qid'";
	$ret_res = mysqli_query($ret_qry,$conn);
	if(mysqli_num_rows($ret_res)>0){
...
?>
    ...
    <div id="toggleText4" style="display: block">
		<table  border="0" cellspacing="1" cellpadding="5" width="100%">
        <tr>
        	<td colspan="2" align="left"><b>Product Posting Status : </b>
            		<?php if($row["pp_status"]=="NO"){ ?>
            		<input type="button" name="ppstatus" id="ppstatus" value="Posting" onClick="javascript: formget(this.form, 'accept/processUpdateTask.php?status=true&action=posting');"
                    <?php } else { ?>
                    <input type="button" name="ppstatus" id="ppstatus" value="DONE" disabled>
                    <?php } ?>
                   	
            </td>
        </tr>
        <tr>
        	<td colspan="2" align="left"><b>Task Status : </b>
                   	<select name="taskstatus" id="taskstatus" onChange="javascript: formget(this.form, 'accept/processUpdateTask.php?status=true&action=task');">
                	<?php if($row["task_status"]=="DONE"){ ?>
...
?>
	<table class="bd" border="0" cellspacing="1" cellpadding="4">
		<tr>
			<td><input type="button" name="btnBack" value="Back" onClick="javascript:LoadDiv('<?php echo CENTER_DIV ?>','accept/task.php?status=true&status=true')"></td>
			<?php 
			if ($recid > 0) 
			{ 
			?>
			<td><input type="button" name="btnPrior" value="Prior Record" onClick="javascript:LoadDiv('<?php echo CENTER_DIV ?>','accept/task.php?status=true&a=<?php echo $a ?>&recid=<?php echo $recid - 1 ?>')"></td>
			<?php 
			} 
			if ($recid < $count - 1) { ?>
				<td>
                <input type="button" name="btnNext" value="Next Record" onClick="javascript:LoadDiv('<?php echo CENTER_DIV ?>','accept/task.php?status=true&a=<?php echo $a ?>&recid=<?php echo $recid + 1 ?>')"></td>
	  <?php } ?>
	   </tr>
	</table>
	<hr size="1" noshade>
...
?>
	<table class="bd" border="0" cellspacing="1" cellpadding="4">
		<tr>
			<td><input type="button" name="btnBack" value="Back" onClick="javascript:LoadDiv('<?php echo CENTER_DIV ?>','accept/task.php?status=true')"></td>
		</tr>
	</table>
	<hr size="1" noshade>
...
?>
    ...
    <form name="frmLead" enctype="multipart/form-data" action="accept/processAccept.php?status=true&action=update" method="post">
		...
		<br>
		<?php if($row["task_status"] == "NOT DONE"){ ?>
		<hr size="1" noshade>
		<table class="bd" border="0" cellspacing="1" cellpadding="4">
			<tr>
				<td>
           			<input type="button" name="btnEdit" onClick="javascript:LoadDiv('<?php echo CENTER_DIV ?>','accept/task.php?status=true&a=edit&filter=<?php echo $prodcode ?>&filter_field=<?php echo $field ?>')" value="Add New Task">
...
?>
	<br>
	<form name="frmLead" enctype="multipart/form-data" action="accept/processTask.php?status=true&action=update" method="post">
		...
		<p>
        <input type="button" name="btnedit" id="btnedit" value="Update" onClick="if(task_lead_validate()==true){javascript: formget(this.form, 'accept/processTask.php');}">
        </p>
        <div id="output" style="color: blue;"></div>
...
?>
	<br>
	<form name="frmLead" action="accept/processAccept.php?status=true&action=delete" method="post">
...
function sql_select()
{
	...
	if($_SESSION['User_Design']=='ADMIN') {
		...
		$sql = "select * from (select c.prod_desc, b.customer_name, b.customer_address,b.customer_city,b.customer_country, b.customer_mobno,
				...
				c.total_amt,c.discount_amt,c.remark,c.task_status,c.pp_status 
