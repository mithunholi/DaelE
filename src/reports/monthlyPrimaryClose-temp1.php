			...
			while($dist_row = mysql_fetch_assoc($dist_result)){
				...
				for($i=0;$i<sizeof($catDatas);$i++){
					...
					while($j<($cntDatas[$i]+1)){
						...
						$pid = $prodCode[$j][$i];
						if($lastvisitdate != ""){
								$priCSData[$i][$j] = primaryOpeningStock($pid,$userId,$distid,$lastvisitdate );
								$transitData[$i][$j] = primaryTransit($pid,$userId,$distid,$lastvisitdate );
								//$deliverData[$i][$j] = primaryDeliver($pid,$userId,$distid,$lastvisitdate );
								//$secSalesData[$i][$j] = secondarySales($pid,$userId,$distid,$lastvisitdate );
						}else{
								$priCSData[$i][$j] =0;
								$transitData[$i][$j]=0;
								//$deliverData[$i][$j]=0;
						...
						if($nextdate != '0'){
							$primaryInvoice[$i][$j] = primaryInvoiceData($pid,$userId,$distid,$nextdate,$todate);
						}else{
				...
				echo "<td rowspan='3'>Last Visit-".date("d-m-Y",strtotime($lastvisitdate))."</td>";
