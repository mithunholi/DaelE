    ...
    <form name="frmFilter" action="" method="post">
 	   <table border="1" cellspacing="0" cellpadding="4" width="100%" bgcolor="#EAEAEA">
         ...
         <tr>
            ...
            <td align="left">
				<select name="cboUser" class="box" id="cboUser" onChange="javascript:LoadDiv('<?php echo CENTER_DIV ?>','reports/monthlyPrimary-temp.php?userId='+this.value+'&fromdate='+document.frmFilter.fromdate.value+'&todate='+document.frmFilter.todate.value)">
         ...
         <tr>
         	<td colspan="2" align="left">
            	<input type="button" name="action" value="Apply" onClick="javascript:findData1(this.form,'reports/monthlyPrimary-temp.php?fromdate='+document.frmFilter.fromdate.value+'&todate='+document.frmFilter.todate.value)">
			 	<input type="button" name="action" value="Reset" onClick="javascript:LoadDiv('<?php echo CENTER_DIV ?>','reports/monthlyPrimary-temp.php?a=reset')">
            </td>
        </tr>
        </table>
     </form>
		...
		if(mysql_num_rows($dist_result)>0){
			...
			while($dist_row = mysql_fetch_assoc($dist_result)){
				...
				for($i=0;$i<sizeof($catDatas);$i++){
					$j=0;
					while($j<($cntDatas[$i]+1)){
						$accSALESPS[$i][$j] = $totSALESPENDqty[$i][$j] + $totSALESREJECTqty[$i][$j] + $totSALESINVENTqty[$i][$j];
				...
				for($i=0;$i<sizeof($catDatas);$i++){
					$j=0;
					while($j<($cntDatas[$i]+1)){
						if($totSALESPENDEDITqty[$i][$j]==0 or $totSALESPENDEDITqty[$i][$j]=="0"){
							echo "<td>-</td>";
						}else if($totSALESPENDEDITqty[$i][$j]>0){
							echo "<td>+".$totSALESPENDEDITqty[$i][$j]."</td>";
						}else{
							echo "<td>".$totSALESPENDEDITqty[$i][$j]."</td>";
						}
						$j++;
				...
				for($i=0;$i<sizeof($catDatas);$i++){
					$j=0;
					while($j<($cntDatas[$i]+1)){
						if($totSALESPENDqty[$i][$j]=="0" or $totSALESPENDqty[$i][$j]==0){
							echo "<td>-</td>";
						}else{
							echo "<td>".$totSALESPENDqty[$i][$j]."</td>";
						}
						$j++;
					}
				}
				echo "</tr>";
					...
					for($i=0;$i<sizeof($catDatas);$i++){
						$j=0;
						while($j<($cntDatas[$i]+1)){
							if($totSALESINVENTqty[$i][$j]=="0" or $totSALESINVENTqty[$i][$j]==0){
								echo "<td>-</td>";
							}else{
								echo "<td>".$totSALESINVENTqty[$i][$j]."</td>";
							}
							$j++;
						}
					}
					...
					for($i=0;$i<sizeof($catDatas);$i++){
						$j=0;
						while($j<($cntDatas[$i]+1)){
							if($totSALESREJECTqty[$i][$j]=="0" or $totSALESREJECTqty[$i][$j]==0){
								echo "<td>-</td>";
							}else{
								echo "<td>".$totSALESREJECTqty[$i][$j]."</td>";
							}
							$j++;
						}
					}
					...
					for($i=0;$i<sizeof($catDatas);$i++){
						$j=0;
						while($j<($cntDatas[$i]+1)){
							$accWEBPS[$i][$j] = $totWEBPENDqty[$i][$j] + $totWEBREJECTqty[$i][$j] + $totWEBINVENTqty[$i][$j];
							if($accWEBPS[$i][$j]=="0" or $accWEBPS[$i][$j]==0){
								echo "<td>-</td>";
							}else{
								echo "<td>".$accWEBPS[$i][$j]."</td>";
							}
							$j++;
						}
					}
					...
					for($i=0;$i<sizeof($catDatas);$i++){
						$j=0;
						while($j<($cntDatas[$i]+1)){
							if($totWEBPENDEDITqty[$i][$j]==0 or $totWEBPENDEDITqty[$i][$j]=="0"){
								echo "<td>-</td>";
							}else if($totWEBPENDEDITqty[$i][$j]>0){
								echo "<td>+".$totWEBPENDEDITqty[$i][$j]."</td>";
							}else{
								echo "<td>".$totWEBPENDEDITqty[$i][$j]."</td>";
							}
					...
					for($i=0;$i<sizeof($catDatas);$i++){
						$j=0;
						while($j<($cntDatas[$i]+1)){
							if($totWEBPENDqty[$i][$j]=="0" or $totWEBPENDqty[$i][$j]==0){
								echo "<td>-</td>";
							}else{
								echo "<td>".$totWEBPENDqty[$i][$j]."</td>";
							}
							$j++;
						}
					}
					...
					for($i=0;$i<sizeof($catDatas);$i++){
						$j=0;
						while($j<($cntDatas[$i]+1)){
							if($totWEBINVENTqty[$i][$j]=="0" or $totWEBINVENTqty[$i][$j]==0){
								echo "<td>-</td>";
							}else{
								echo "<td>".$totWEBINVENTqty[$i][$j]."</td>";
							}
							$j++;
						}
					}
					...
					for($i=0;$i<sizeof($catDatas);$i++){
						$j=0;
						while($j<($cntDatas[$i]+1)){
							if($totWEBREJECTqty[$i][$j]=="0" or $totWEBREJECTqty[$i][$j]==0){
								echo "<td>-</td>";
							}else{
								echo "<td>".$totWEBREJECTqty[$i][$j]."</td>";
							}
							$j++;
						}
					}
					...
					for($i=0;$i<sizeof($catDatas);$i++){
						...
						while($j<($cntDatas[$i]+1)){
							...
							//$totINVqty[$i][$j] = $totSALESINVENTqty[$i][$j] + $totWEBINVENTqty[$i][$j];
							
							//$grantINVqty[$i][$j] = $grantINVqty[$i][$j] + $totINVqty[$i][$j]; 
							if($totINVqty[$i][$j]=="0" or $totINVqty[$i][$j]==0){
								echo "<td>-</td>";
							}else{
								echo "<td>".$totINVqty[$i][$j]."</td>";
							}
							$j++;
						}//while
					}//for
					...
					for($i=0;$i<sizeof($catDatas);$i++){
								$j=0;
						while($j<($cntDatas[$i]+1)){
							if($totTRANSITqty[$i][$j]=="0" or $totTRANSITqty[$i][$j]==0){
								echo "<td>-</td>";
							}else{
								echo "<td>".$totTRANSITqty[$i][$j]."</td>";
							}
							$j++;
						}
					}
					...
					for($i=0;$i<sizeof($catDatas);$i++){
						$j=0;
						while($j<($cntDatas[$i]+1)){
							if($totDONEqty[$i][$j]=="0" or $totDONEqty[$i][$j]==0){
								echo "<td>-</td>";
							}else{
								echo "<td>".$totDONEqty[$i][$j]."</td>";
							}
							$j++;
						}
					}
					...
					for($i=0;$i<sizeof($catDatas);$i++){
						$j=0;
						while($j<($cntDatas[$i]+1)){
							$totqty = $totDONEqty[$i][$j] + $totTRANSITqty[$i][$j] + $totINVqty[$i][$j];
...
function callSalesmanPendingEdit($did,$uid,$fdate,$tdate,$ustatus,$dstatus,$catData,$cntData,$prodCodes){
	...
	for($i=0;$i<sizeof($catData);$i++){
		...
		while($j<($cntData[$i]+1)){
			...
			//$pname = $prodDatas[$j][$i];
			$pcode = $prodCodes[$j][$i];
			$obsql = "select * from (select a.prod_id, sum(a.eqty) accqty from tbl_orderbook a,tbl_orderbook_child b
								where a.orderid=b.orderid and b.userid='$uid' and b.distid='$did' and a.prod_id = '$pcode' and a.categoryid='$catcode' 
								and b.ordertype = 'ORDER BOOKING' and b.user_status = '$ustatus' and b.order_status='PENDING' 
