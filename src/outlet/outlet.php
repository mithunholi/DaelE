	...
	<table class="bd" border="0" cellspacing="1" cellpadding="4" width="100%">
		...
		<tr>
			<td align="left">
            	...
            	<input type="button" name="btnDelete" value="Delete" onClick="if(onDeletes()==true){ javascript:formget(this.form,'outlet/processOutlet.php'); javascript:LoadDiv('<?php echo CENTER_DIV ?>','outlet/outlet.php?status=true');javascript:printHeader('Outlet Admin');}" >
	...
	<table class="tbl" border="0" cellspacing="1" cellpadding="5" width="75%">
          ...
          <tr> 
   			<td width="150" class="hr">Area Search:</td>
   				<td align="left"> 
                	<select name="cboArea" id="cboArea" class="box" onChange="javascript:LoadDiv('<?php echo CENTER_DIV ?>','outlet/outlet.php?a=<?php echo $aa; ?>&iseditmode=<?php echo $mode; ?>&areaId='+this.value)">
                    <?php
						echo $areaList;
					?>
     			</td>
...
function sql_select()
{
	...
	$sql = "SELECT * FROM (SELECT concat(c.region_name,'',b.area_name) dist_code, concat(concat(c.region_name,'',b.area_name),'-',b.area_desc1) regarea, a.*,
			...
			FROM tbl_outlet a,tbl_area b,tbl_region c,tbl_distributor d,tbl_route e 
