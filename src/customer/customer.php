...
?>
				...
				<input type="text" name="filter" class="uppercase" value="<?php echo $filter ?>">
                  ...
                  <select name="filter_field">
                    ...
                    <option value="<?php echo "customer_mobno" ?>"<?php if ($filterfield == "customer_mobno") { echo "selected"; } ?>>
                     ...
                     <option value="<?php echo "customer_email" ?>"<?php if ($filterfield == "customer_email") { echo "selected"; } ?>>
						<?php echo htmlspecialchars("Email") ?>
                    </option>
				 </select>
               
				<input type="button" name="action" value="Apply" onClick="javascript:SearchData(this.form,'customer/customer.php?status=true&a=filter')" >
				<input type="button" name="action" value="Reset" onClick="javascript:LoadDiv('<?php echo CENTER_DIV ?>','customer/customer.php?status=true&a=reset')"></td>
		     </tr>
			
		</table>
	</form>
	<hr size="1" noshade>
    <form name="frmOutlet" id="frmOutlet" action="customer/customer.php?status=true&a=del" method="post">
	<table class="bd" border="0" cellspacing="1" cellpadding="4" width="100%">
    	
		<tr>
			<td align="left">
            	<input type="button" name="action" value="Add New Customer" onClick="javascript:LoadDiv('<?php echo CENTER_DIV ?>','customer/customer.php?status=true&a=add&customerId=<?php echo $customerId ?>')">
            ...
            <td align="right">Rows Per Page
    	     <select name="pageperrecord" onChange="javascript:LoadDiv('<?php echo CENTER_DIV ?>','customer/customer.php?status=true&pageId='+this.value)">
	...
	<table class="tbl" border="0" cellspacing="1" cellpadding="5"width="100%">
        ...
        <td class="hr">S.No.</td>
        <td class="hr"><a class="hr" href="javascript:LoadDiv('<?php echo CENTER_DIV ?>','customer/customer.php?status=true&order=<?php echo "customer_name" ?>&type=<?php echo $ordtypestr ?>')">
			<?php echo htmlspecialchars("Customer Name") ?></a>
        </td>
	
        <td class="hr"><a class="hr" href="javascript:LoadDiv('<?php echo CENTER_DIV ?>','customer/customer.php?status=true&order=<?php echo "customer_cperson" ?>&type=<?php echo $ordtypestr ?>')">
			<?php echo htmlspecialchars("Contact Person") ?></a>
        </td>
		<td class="hr"><a class="hr" href="javascript:LoadDiv('<?php echo CENTER_DIV ?>','customer/customer.php?status=true&order=<?php echo "customer_mobno" ?>&type=<?php echo $ordtypestr ?>')">
			<?php echo htmlspecialchars("Mobile No") ?></a>
        </td>
        <td class="hr"><a class="hr" href="javascript:LoadDiv('<?php echo CENTER_DIV ?>','customer/customer.php?status=true&order=<?php echo "customer_email" ?>&type=<?php echo $ordtypestr ?>')">
	...
	?>
		...
		<tr class="<?php echo $style ?>" style="cursor:pointer">
            ...
            <td align="left" onClick="javascript:LoadDiv('<?php echo CENTER_DIV ?>','customer/customer.php?status=true&a=view&filter=<?php echo $outcode; ?>&filter_field=<?php echo $field ?>')">
...
?>
	<table class="tbl" border="0" cellspacing="1" cellpadding="5" width="75%">
        ...
        <tr>
			...
			<td class="dr" align="left">
            	<input type="text" class="lowercase" id="customer_email1" name="customer_email1" maxlength="30" size="30" value = "<?php echo $row["customer_email"] ?>">
                <?php echo htmlspecialchars("(optional)")."&nbsp;" ?>
            </td>
		</tr>
        <tr>
			...
			<td class="dr" align="left">
            	<input type="text" class="lowercase" id="customer_web" name="customer_web" maxlength="30" size="30" value = "<?php echo $row["customer_web"] ?>">
                <?php echo htmlspecialchars("(optional)")."&nbsp;" ?>
            </td>
		</tr>
...
function showrow($row, $recid)
{
  ...
  $customeremail1 = $row["customer_email"];
  $customerweb = $row["customer_web"];
...
?>
	<table class="tbl" border="0" cellspacing="1" cellpadding="5"width="50%">
        ...
        <tr>
         ...
         <tr>
			<td class="hr" align="left"><?php echo htmlspecialchars("Website")."&nbsp;" ?></td>
			<td class="dr" align="left"><?php echo htmlspecialchars($customerweb) ?></td>
		</tr>
	</table>
<?php 
} 
...
?>
	<table class="bd" border="0" cellspacing="1" cellpadding="4">
		<tr>
			...
			{ 
			?>
				<td><input type="button" name="btnPrior" value="Prior Record" onClick="javascript:LoadDiv('<?php echo CENTER_DIV ?>','customer/customer.php?status=true&a=<?php echo $a ?>&recid=<?php echo $recid - 1 ?>')"></td>
			<?php 
			} 
			if ($recid < $count - 1) { ?>
				<td>
                	<input type="button" name="btnNext" value="Next Record" 
                    	onClick="javascript:LoadDiv('<?php echo CENTER_DIV ?>','customer/customer.php?status=true&a=<?php echo $a ?>&recid=<?php echo $recid + 1 ?>')">
                </td>
	  <?php } ?>
	   </tr>
	</table>
...
?>
	...
	<hr size="1" noshade>
	<form name="customer_validate" enctype="multipart/form-data" action="customer/processCustomer.php?status=true&action=add" method="post">
...
?>
	...
	<table class="bd" border="0" cellspacing="1" cellpadding="4">
		<tr>
			<td><input type="button" name="btnEdit" value="Edit Record" onClick="javascript:LoadDiv('<?php echo CENTER_DIV ?>','customer/customer.php?status=true&a=edit&filter=<?php echo $customercode ?>&filter_field=<?php echo $field ?>')"></td>
		</tr>
	</table>
	<?php
  		mysqli_free_result($res);
...
?>
	<br>
	<form name="customer_validate" enctype="multipart/form-data" action="customer/processCustomer.php?status=true&action=update" method="post">
...
?>
	<br>
	<form action="customer/processCustomer.php?status=true&action=delete" method="post">
...
function sql_select()
{
	...
	$sql = "SELECT * FROM tbl_customer WHERE 1 ".$queryString;
	...
	if (isset($filterstr) && $filterstr!='' && isset($filterfield) && $filterfield!='') {
    	...
    	$sql .= " and (`customer_name` like '" .$filterstr ."') or (`customer_cperson` like '" .$filterstr ."')  
				or (`customer_mobno` like '" .$filterstr ."') or (`customer_email` like '" .$filterstr ."')";
  	}
...
function sql_getrecordcount()
{
	...
	$sql = "SELECT count(*) FROM tbl_customer WHERE 1 ".$queryString;
	...
	if (isset($filterstr) && $filterstr!='' && isset($filterfield) && $filterfield!='') {
    	...
    	$sql .= " and (`customer_name` like '" .$filterstr ."') or (`customer_cperson` like '" .$filterstr ."') or (`customer_email` like '" .$filterstr ."') 
				or (`customer_mobno` like '" .$filterstr ."')";
  	}
