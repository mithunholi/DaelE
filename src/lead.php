...
?>
	...
	<tr class="<?php echo $style ?>" style="cursor:pointer">
      ...
      <td align="center" onClick="javascript:LoadDiv('<?php echo CENTER_DIV ?>','lead/lead.php?a=view&recid=<?php echo $i ?>')">
...
?>
	...
	<table  border="0" cellspacing="1" cellpadding="5" width="100%" class="hrhighlightblue">
        ...
        <tr>
			...
			<td align="left">
            	<?php if($iseditmode == 1) { ?>
                <textarea style="width:350px; height:80px; text-align:left; vertical-align:top;" cols="42" rows="5" name="cust_add"><?=$cust_row["customer_address"]?></textarea>
                <?php }else{ ?>
                <textarea name="cust_add" cols="80" rows="5"><?=$cust_row["customer_address"]?></textarea>
                <?php } ?>
            ...
            <?php 
				if($iseditmode != 1){ 
            		$fupdate = date("d-m-Y",strtotime($today));
			 		...
			 		$fupdate = date("d-m-Y",strtotime($row['lead_book_date']));
			 	} 
			?>
    ...
    <form name="frmLead" enctype="multipart/form-data" action="lead/processLead.php?action=update" method="post">
		...
		<table class="bd" border="0" cellspacing="1" cellpadding="4">
			<tr>
				...
				<td>
            		<input type="button" name="btnReject" onClick="javascript:LoadDiv('<?php echo CENTER_DIV ?>','lead/lead.php?a=reject&filter=<?php echo $prodcode ?>&filter_field=<?php echo $field ?>')" value="Reject Record">
